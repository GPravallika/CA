...
export default class extends React.Component{
  ...
  let userDetails = {}; 
    ...
    if (this.showFormErrors()) {
      ...
      userDetails.token = this.props.params.resetPasswordToken;
      let regSrvUpdPassRes = null;
      RegistrationService.updateUserPassword(userDetails)
      .then((response) => {
        regSrvUpdPassRes = response.clone();
        return response.json();
      })
      .then((responseData) => {
        if(regSrvUpdPassRes.ok) {
  ...
  showInputError(refName) {
    ...
    if (!validity.valid) {
      if (validity.valueMissing) {
        ...
        error.textContent = `${label} should be between 5-15`; 
