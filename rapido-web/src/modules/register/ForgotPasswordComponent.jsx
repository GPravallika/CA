...
export default class extends React.Component{
  ...
  handleSubmit(e) {    
    ...
    if (this.showFormErrors()) {
      let regSrvRstUsrDetRes = null;
      RegistrationService.resetUserDetails(this.state.email)
      .then((response) => {
        regSrvRstUsrDetRes = response.clone();
        return response.json();
      })
      .then((responseData) => {
        if(regSrvRstUsrDetRes.ok) {
          this.setState({
            resetPasswordLinkSent: true
          });
        } else {
          showAlert(this, (responseData.message) ? responseData.message : "Error occured");
          if(regSrvUpdPassRes.status == 401) {
  ...
  /* Render Method */
  render() {
    let forgotPassowrdContent;
    if(this.state.resetPasswordLinkSent) {
      forgotPassowrdContent = <p className="resetLinkSuccessMessage">
        A mail has been sent to registered Email to reset your account password. Please reset and <Link to="/login"><span className="login-label">Login</span></Link>
      </p>;
    } else {
      forgotPassowrdContent = <div>
        ...
        <div className="form-group registration-options">
          <button className="btn btn-default form-control reset-button"
          onClick={ this.handleSubmit }>Reset</button>
          <button className="btn btn-default form-control cancel-button"
